...
try {
  ...
  console.error('AWS.config.secretAccessKey = ' + getEnvVariable('VCO_BACKUPS_SECRET_ACCESS_KEY') ) ;
  console.error('AWS.config.region = ' + getEnvVariable('VCO_BACKUPS_REGION') ) ;
  ...
  var AWS_STATIC_SOURCE_DIRECTORY = 'backup';
...
var BACKUP_PATH = path.join(VCO_BACKUPS_PREFIX, new Date().toISOString().split('T')[0], SOURCE_VERSION);
...
fs.readFile( PUBLIC_ASSETS_SOURCE_DIRECTORY + '/data.txt', function (err, file) {
  if (err) { throw err; }


  console.log('Data to upload on new function:', file);

  var contentType = mimeTypes.lookup(path.extname(PUBLIC_ASSETS_SOURCE_DIRECTORY + '/data.txt')) || null;
  ...
  s3.upload({
    ACL: 'public-read',
    //Key: path.join(BACKUP_PATH, file.replace(PUBLIC_ASSETS_SOURCE_DIRECTORY, '')),
    Key: BACKUP_PATH,
    Body: base64data,
